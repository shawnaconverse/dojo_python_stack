...
class Dog:
    ...
    # dont need reference to the class or an instance
    # we put this method in the Dog class cause it validates dog data
    @staticmethod 
    def validate_dog(post_data): # post_data is the data from request.form
        is_valid = True # we assume the data is valid at the start

        if len(post_data['name']) < 2:
            # Flash message - exists for only one cycle of HTTP req / res
            flash("Dog name must be at least 2 characters.")
            is_valid = False

        if len(post_data['age']) < 1: # checking to make sure they entered an age at all
            flash("Dogs must have an age.")
            is_valid = False
            
        if len(post_data['hair_color']) < 1:
            flash("Dogs must have a hair color.")
            is_valid = False

        return is_valid # return the boolean whether data is valid or not
